...
class HybridRAGSearch:
    def __init__(self, persist_dir: str = "chroma_db", data_dir: str = "data", embedding_model: str = "nomic-ai/nomic-embed-text-v1.5", dense_weight: float = 0.6, sparse_weight: float = 0.4, reranker=None):
        ...
        if self.vectorstore.collection_exists():
            ...
            docs, raw_tables, failures = load_all_documents(self.data_dir)
    ...
    def retrieve(self, query: str, top_k: int = 5):
        ...
        # Swap in the full table HTML for any table hit
        results = self.vectorstore.resolve_full_content(results, self.table_store)
    ...
    def add_document(self, file_path: str) -> dict:
        ...
        content-hashed ID (or by table_id for table chunks), so re-adding
        only touches the new file's chunks - nothing existing gets
        re-embedded or disturbed.
